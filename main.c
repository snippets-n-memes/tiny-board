...
void run(){
  ...
  Ticket test2 = *newTicket(menu[0],"test chain", "test tickets adding an addtional ticket, with a long description.");
  Ticket test3 = *newTicket(menu[0],"test chain", "overwrite... adding an addtional ticket, with a long description.");
  Ticket test4 = *newTicket(menu[1],"test chain", "make decision about managing tickets as files, or by writing driver code in go and manage in memory");
...
void selectTicket(Ticket *ticket) {
  ...
  wmove(t.menu, ++y, WWIDTH - 2);
  wvline(t.menu, ACS_VLINE, t.size+4);
  wmove(t.menu, y, 1);
  wvline(t.menu, ACS_VLINE, t.size+4);

  wmove(t.menu, y+t.size+3, 1);
  waddch(t.menu, ACS_LLCORNER);
  whline(t.menu, ACS_HLINE, WWIDTH - 4);
  wmove(t.menu, y+t.size+3, WWIDTH - 2);
